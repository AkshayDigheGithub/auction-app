...
import { DataTable, ExportButton, Pager, SearchInput, SectionTitle, Select, Stat, Td, Th } from "./shared";
...
interface ShopDetail {
  ...
  recentDeals: { id: string; finalPrice: string; feeAmountPaise: number | null; feeStatus: string; qrStatus: string; request: { productName: string } }[];
}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="grid gap-2 sm:grid-cols-3">
        <SearchInput value={q} onChange={(v) => { setQ(v); setSkip(0); }} placeholder="Search name, address, phone…" />
        <Select
          value={category}
          onChange={(v) => { setCategory(v); setSkip(0); }}
          allLabel="All categories"
          options={Object.entries(SHOP_CATEGORY_LABELS).map(([value, label]) => ({ value, label }))}
        />
        <div className="flex items-center gap-2">
          <label className="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-300">
            <input type="checkbox" checked={lowOnly} onChange={(e) => { setLowOnly(e.target.checked); setSkip(0); }} />
            Low balance only
          </label>
          ...
          >
            {data.rows.map((s) => (
              <tr key={s.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/40">
                ...
                <Td className="text-neutral-600 dark:text-neutral-300">{categoryLabel(s.category)}</Td>
                <Td align="right" className={`tabular-nums ${s.lowBalance ? "font-semibold text-amber-700 dark:text-amber-400" : ""}`}>
                ...
                <Td align="right" className="tabular-nums text-neutral-600 dark:text-neutral-300">{s._count.deals}</Td>
                <Td align="right">
...
function ShopDetailPanel({
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end bg-black/30" onClick={onClose}>
      <aside
        className="h-full w-full max-w-lg overflow-y-auto bg-white px-5 py-6 shadow-xl dark:bg-neutral-950"
              ...
              <ul className="flex flex-col gap-1.5 text-sm">
                {detail.recentDeals.map((d) => (
                  <li key={d.id} className="flex items-baseline justify-between gap-3">
                    <span className="truncate text-neutral-600 dark:text-neutral-300">{d.request.productName}</span>
